...
void Problemi::cec19_test_func(double *x, double *f, int nx, int mx, int func_num)
{
	int cf_num = 10, i, j;
	if (ini_flag == 1)
	{
...
void Problemi::schaffer_F7_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag) /* Schwefel's 1.2  */
{
	int i;
	double tmp;
	f[0] = 0.0;
...
void Problemi::griewank_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag) /* Griewank's  */
{
	int i;
	double s, p;
	s = 0.0;
...
void Problemi::ackley_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag) /* Ackley's  */
{
	int i;
	double sum1, sum2;
	sum1 = 0.0;
...
void Problemi::weierstrass_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag) /* Weierstrass's  */
{
	int i, j, k_max;
	double sum, sum2, a, b;
	a = 0.5;
...
void Problemi::rastrigin_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag) /* Rastrigin's  */
{
	int i;
	f[0] = 0.0;
...
void Problemi::schwefel_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag) /* Schwefel's  */
{
	int i;
	double tmp;
	f[0] = 0.0;
...
void Problemi::escaffer6_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag) /* Expanded Scaffer¡¯s F6  */
{
	int i;
	double temp1, temp2;
...
void Problemi::happycat_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag) /* HappyCat, provdided by Hans-Georg Beyer (HGB) */
{
	int i;
...
void Problemi::shiftfunc(double *x, double *xshift, int nx, double *Os)
{
	int i;
	for (i = 0; i<nx; i++)
	{
...
void Problemi::rotatefunc(double *x, double *xrot, int nx, double *Mr)
{
	int i, j;
	for (i = 0; i<nx; i++)
	{
...
void Problemi::sr_func(double *x, double *sr_x, int nx, double *Os, double *Mr, double sh_rate, int s_flag, int r_flag) /* shift and rotate */
{
	int i;
	if (s_flag == 1)
	{
...
void Problemi::asyfunc(double *x, double *xasy, int nx, double beta)
{
	int i;
	for (i = 0; i<nx; i++)
	{
...
void Problemi::oszfunc(double *x, double *xosz, int nx)
{
	int i, sx;
	double c1, c2, xx;
	for (i = 0; i<nx; i++)
...
void Problemi::Lennard_Jones(double *x, int D, double *f)  // find the atomic configuration with minimum energy
{
...
void Problemi::Hilbert(double *x, int D, double *f)  // find the inverse of the (ill-conditioned) Hilbert matrix
{
...
void Problemi::Chebyshev(double *x, int D, double *f)  // Storn's Tchebychev - a 2nd ICEO function - generalized version
{
	/* Valid for any D>2
	constraints: unconstrained
	type: multi-modal with one global minimum; non-separable
